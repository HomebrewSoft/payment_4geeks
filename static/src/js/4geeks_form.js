odoo.define('payment_4geeks.payment_4geeks', function (require) {
    "use strict";
    ...
    $(document).ready(function () {
        ...
        $('#o_payment_form_pay').on('click', function (ev) {
            ...
            if (provider != "4geeks" || is_form_payment !== true) {
                return;
            }

            var temp = 0;
            var loader = "<div class='p4geeks_payment_loder' style='display:none;'></div>";
            $('.o_payment_form').after(loader);

            if (temp == 0) {
                var amount = $('#p4geeks-checkout').find('input[name="amount"]').val();
                temp = 1;

                $('.p4geeks_payment_loder').show();
                if (!$modal) {
                    ajax.jsonRpc("/p4geeks/modal", 'call', {}).then(function (modal) {
                        $modal = $(modal);
        ...
        $payment.on("click", "input[name='pm_id']", function (ev) {
            ...
            if (provider !== "4geeks" && ($modal)) {
                $modal.hide();
                $('#o_payment_form_pay').show();
                return;
            } else if (provider === "4geeks" && ($modal)) {
                $modal.show();
                $('#o_payment_form_pay').hide();
                return;
            }
